...
For the SLT tasks, RWTH reintroduced punctuation and case information before the actual translation similar to \cite{peitz:iwslt2011}.
...
The UEDIN systems were trained using monolingual and parallel data from
...
as provided by the Linguistic Data Consortium~\cite{ldc:www}, and the News
Crawl, $10^9$, and News Commentary corpora from the WMT shared task training
...
\end{packed_itemize}
%
Model weights for the log-linear model combination~\cite{och02:meForSMT} were
optimized with batch MIRA~\cite{cherry:2012:bts} to maximize
\BLEU~\cite{papineni02:bam}.

\subsection{Spoken Language Translation System}
\label{uedin:slt}

We trained a phrase-based German$\to$English SLT system for this system combination paper.
...
capitalisation and this is one
...
what we did by training a translation system on the German side of our parallel data.
...
Our handling of punctuation is similar to the other groups in this paper, however we used a phrase-based
model with no distortion or reordering, and we tuned the model to the ASR input text using
...
\subsection{Edinburgh's MT Systems}


\subsubsection{German$\to$English}

Pre-reordering \cite{collins-koehn-kucerova:2005:ACL} and compound splitting
\cite{ref:koehn2003a} were applied to the German source language side in a
preprocessing step.  A factored translation model \cite{koehn2012factor} was
employed. Source side factors are word, lemma, part-of-speech tag, and
morphological tag. Target side factors are word, lemma, and part-of-speech tag.

%In addition to the features listed above, 
UEDIN incorporated several additional
features into the German$\to$English MT system, in particular:
\begin{packed_itemize}
\item A 7-gram LM over POS tags (trained on WIT$^3$ only)
\item A 7-gram LM over lemmas (trained on WIT$^3$ only)
\item OSMs over different word representations~\cite{durrani-EtAl:2014:Coling} 
\end{packed_itemize}

\noindent A concatenation of dev2010 and dev2012 was utilized as development set.


\subsubsection{English$\to$French}
...
\begin{packed_itemize}
\item LMs over Brown clusters \\(UEDIN-A: 7-gram, UEDIN-B: 5-gram)
\item OSMs over words and over Brown clusters
\end{packed_itemize}

\noindent Model weights were tuned on dev2010.

\subsubsection{English$\to$German}

\noindent \textit{Phrase-based systems.}

\noindent \textit{Syntax-based system.}
...
In this section, we will give a short re-introduction of confusion network system combination.
